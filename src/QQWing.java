...
public class QQWing {
  ...
  //      reset();		We need the solution also
...
*/
            ...
            while (!done){
                ...
                if (action == GENERATE){
                    ...
                    // Read the puzzle input from panel1
                    int[] puzzle = new int[BOARD_SIZE];
                    for(int j=0;j<81;j++)
                    puzzle[j]=panel1.ques[j];
    ...
    /*		int p=0;
    		for(int x=0;x<9;x++)	
			{
				for(int y=0;y<9;y++)
				{
					System.out.print(ss.solution[p]);
					p++;
				}
				System.out.println();
			}
	*/
    		...
    		this.puzzle=ss.puzzle;
			new2.diff=ss.getDifficulty();
			System.out.println(ss.getDifficulty());
